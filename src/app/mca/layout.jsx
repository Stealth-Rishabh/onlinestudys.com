export const metadata = {
  title: "Best Online Universities and Colleges for MCA- Online Studys",
  description:
    "Discover the best online universities & colleges for MCA -Online Studys - We offer cost effective price with proper amenities and highly qualified mentors, faculties get the chance to learn on live classes",
  keywords:
    "online MCA, MCA distance learning, MCA programs India, computer applications degree, online MCA courses",
  openGraph: {
    title: "Best Online Universities and Colleges for MCA- Online Studys",
    description:
      "Discover the best online universities & colleges for MCA -Online Studys - We offer cost effective price with proper amenities and highly qualified mentors, faculties get the chance to learn on live classes",
    type: "website",
    url: "https://onlinestudys.com/mca",
    siteName: "Online Studys",
  },
  twitter: {
    card: "summary_large_image",
    title: "Best Online Universities and Colleges for MCA- Online Studys",
    description:
      "Discover the  best online universities & colleges for MCA -Online Studys -  We offer cost effective price with proper amenities and highly qualified mentors ,  faculties get the chance to learn on live classes",
  },
  robots: {
    index: true,
    follow: true,
  },
};

export default function McaLayout({ children }) {
  return children;
}
